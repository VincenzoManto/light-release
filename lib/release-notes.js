 ...
 * @returns {Object} The generated release notes content in HTML and Markdown format
 */
function generateReleaseNotes(version, config) {
  ...
  let htmlContent, mdContent, aiComment;
  
  if (config.aiComment?.enabled) {
    const commits = getCommits(config);
    if (commits.length) {
      try {
        if (config.aiComment.openAIKey) {
          process.env.OPENAI_API_KEY = config.aiComment.openAIKey;
        }
        let prompt = `"Create release note commentary based on these commit messages.\n${config.aiComment.additionalPrompt}:\n${commits.map(commit => commit.message).join('\n -')}"`;
        prompt = prompt.replace(/\n/g, '\\n');
        const aiCommentCommand = `npx llm --quiet -T 100 -t 0.5 ${prompt}`;
        aiComment = execSync(aiCommentCommand, { encoding: 'utf-8' });
        if ((/^\w+Error:/.test(aiComment))) {
          aiComment = '';
          throw new Error(aiComment);
        }
      } catch (error) {
  ...
  if (config.generateHTML) {
    htmlContent = generateHTMLContent(version, formattedDate, config, aiComment);
    fs.writeFileSync(path.join(releaseNotesDir, `release.html`), htmlContent);
  }
  if (config.generateMarkdown) {
    mdContent = generateMarkdownContent(version, formattedDate, config, aiComment);
 ...
 * @param {*} config configuration object
 * @param {*} aiComment AI generated comment
 * @returns
 */
function generateHTMLContent(version, date, config, aiComment) {
  const commits = getCommits(config);
  ...
  const versionFilePath = path.join(process.cwd(), config.versionFileName);
  const versionFileJSON = JSON.parse(fs.readFileSync(versionFilePath, 'utf8'));
  const packageName = versionFileJSON.name;
...
<div class="contents">
  <div class="contents":key="key">
      <section class="border-b-[1px] pt-[20px] pb-[20px] border-[color:var(--64ddf9af7c88e)]">
        ...
        ${aiComment ? `<section class="pb-[80px] pt-[80px]"><div class="mt-[0] mr-[auto] mb-[0] ml-[auto] pt-[0] pr-[32px] pb-[0] pl-[32px] md:pr-[24px] md:pl-[24px] max-w-[1024px]"><h3 class="text-[24px] font-semibold tracking-[-0.025em]">Summary</h3><p class="text-[20px] text-[color:var(--64ddf9af7c9a7)]">${aiComment}</p></div></section>` : ''}
 ...
 * @returns
 */
function generateMarkdownContent(version, date, config, aiComment) {
  const commits = getCommits(config);
  let changeLog = '';
  ...
  for (const commit of commits) {
    ...
    committer.name = committer.name?.replace(/.*?\+/g, '');
    committer.image = committer.name ? `https://avatars.githubusercontent.com/${committer.name}?size=40` : null;
    if (config.omitAuthor) {
      committer = {};
    }
    const modifiedFiles = commit.modifiedFiles || 0;
    const impactBadge = config.showCommitImpact ? getImpactBadge(modifiedFiles, config, 'md') : '';

    changeLog += `
### ${commit.message}

${impactBadge}
${!config.omitAuthor ? `- **Author:** [${committer.name}](${committer.profileLink}) ![Author Image](${committer.image || 'https://github.com/github.png?size=40'})` : ''}
- **Date:** ${commit.date}
...
function updateChangelog(version, date, config) {
  ...
  const newEntry = `## [${version}] - ${date}\n\n### Changes\n- Release notes generated for version [${version}](${config.baseUrl || '.'}/.release-notes/${version}/release.md)\n\n`;
