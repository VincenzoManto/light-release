...
function getLastCommit(config) {
  ...
  try {
    const commits = execSync(`git log --format=%H -- ${versionFileName}`, { encoding: 'utf8' })
      .toString()
      .trim()
      .split('\n');

    for (const sha of commits) {
      const content = execSync(`git show ${sha}:${versionFileName}`).toString();
    ...
    console.error('Error reading last commit about versioning:', error.message);
    process.exit(1);
  }
