...
function getCommits() {
  try {
    const lastCommitHash = getLastCommit(); // Replace with the actual commit hash

    const commitLogCommand = lastCommitHash ? `git log ${lastCommitHash}..HEAD --pretty=format:"%H|%s|%ae|%an|%cd" --numstat` : 
    'git log --pretty=format:"%H|%s|%ae|%an|%cd" --numstat';
...
function classifyVersion(commits, config) {
  let major = 0,
    ...
    if (!majorWords) {
        majorWords = [/^feat(\(.*\))?!:/, /BREAKING CHANGE/, /^\s*major|^\s*breaking/i, /^perf(\(.*\))?:/i];
    } else {
